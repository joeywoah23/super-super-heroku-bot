...
rMember.addRoles(gRole).catch(console.error);
    message.channel.send({embed: {
      ...
      title: "RBan Action",
          fields: [{
          name: "Rban",
          value: `**${message.author.tag}**, **${rMember}** has been rolebanned for **${reason}**`
        }
      ],
      footer: {
        icon_url: client.user.avatarURL,
  ...
  });; //Messages the channel that the user was suspended

    try {
      await rMember.send(`You have been rolebanned for **${reason}**.`); //Tries to DM User
    } catch (e) {
...
client.on("message", async (bot, message, args) => {
	...
	if (wUser.hasPermission("MANAGE_MESSAGES")) return message.reply ("You have sufficient permissions to do this command!");
...
if (err) console.log(err);
	...
	let warnchannel = message.guilds.channels.find('name', '🗂》mod-logs');
	if (!warnchannel) return message.reply("Cannot find the mod logs channel!");
	warnchannel.send(warnEmbed)
	
	if (warns[wUser.is].warns == 4){
		let reason = "Reached 4th Infraction"
		member.kick(reason)
		message.channel.send({embed: {
            color: 16231339,
            author: {
            ...
            title: "Kick Action",
                fields: [{
                name: "Kick",
                value: `**${message.author.tag}**, User ${wUser} has been kicked from the server because: **${reason}**.`
              }
            ],
            footer: {
              icon_url: client.user.avatarURL,
	...
	if (warns[wUser.is].warns == 6){
		let reason = "Reached 6th Infraction"
		message.channel.send({embed: {
            color: 16231339,
            author: {
              name: client.user.username,
            ...
            title: "Ban Action",
                fields: [{
                name: "Kick",
                value: `**${message.author.tag}**, User ${wUser} has been banned from the server for **${reason}**.`
              }
            ],
            footer: {
              icon_url: client.user.avatarURL,
